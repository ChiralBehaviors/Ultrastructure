...
public abstract class TwoFormInflector {
        ...
        public String getPlural(String word) {
            ...
            for (String suffix : list) {
                if (lowerWord.endsWith(suffix)) {
                    ...
                    return word.substring(0, word.length() - singular.length())
                           + plural;
                }
            }
            return null;
    ...
    protected void irregular(String singular, String plural) {
        if (singular.charAt(0) == plural.charAt(0)) {
            rules.add(new RegExpRule(Pattern.compile("(?i)("
                                                     + singular.charAt(0) + ")"
    ...
    protected void rule(String singular, String plural) {
        rules.add(new RegExpRule(Pattern.compile(singular,
    ...
    protected void rule(String[][] list) {
        for (String[] pair : list) {
            rules.add(new RegExpRule(Pattern.compile(pair[0],
                                                     Pattern.CASE_INSENSITIVE),
                                     pair[1]));
        }
    }
