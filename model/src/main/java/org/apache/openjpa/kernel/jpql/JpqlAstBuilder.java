...
/**
 * Builder for JPQL expressions. This class takes the query parsed in
 * {@link JPQL} and converts it to an expression tree using an
 * {@link ExpressionFactory}. Public for unit testing purposes.
 * 
 * @author Marc Prud'hommeaux
 * @author Patrick Linskey
 * @nojavadoc
 */
public class JpqlAstBuilder extends AbstractExpressionBuilder implements
        JPQLTreeConstants {

    private static final int       VAR_PATH  = 1;
    private static final int       VAR_ERROR = 2;

    private static final Localizer _loc      = Localizer.forPackage(JpqlAstBuilder.class);
     ...
     *            the original String
     * @param val2
     *            the 1-based start index as per JPQL substring() semantics
     * @param val3
    ...
    public static void setImplicitTypes(Value val1,
        ...
        Parameter param = val1 instanceof Parameter ? (Parameter) val1
                                                   : val2 instanceof Parameter ? (Parameter) val2
                                                                              : null;
        Path path = val1 instanceof Path ? (Path) val1
                                        : val2 instanceof Path ? (Path) val2
                                                              : null;
    ...
    /**
     * Assemble the children of the specific node by appending each child,
    ...
    /**
     * The Nth from the last child. E.g., lastChild(1) will return the
     * second-to-the-last child.
     */
     ...
     * @param query
     *            used to resolve variables, parameters, and class names used in
     *            the query
     * @param parsedQuery
     *            the parsed query
     */
    public JpqlAstBuilder(ExpressionFactory factory, Resolver resolver,
                          Object parsedQuery) {
        super(factory, resolver);

        contexts.push(new Context(
                                  parsedQuery instanceof ParsedJPQL ? (ParsedJPQL) parsedQuery
                                                                   : parsedQuery instanceof String ? getParsedQuery((String) parsedQuery)
                                                                                                  : null,
                                  null, null));
    ...
    public QueryExpressions getQueryExpressions() {
        ...
        exps.hasInExpression = hasParameterizedInExpression;
     ...
     *            the node to check
     * @param inner
     *            whether or not the join should be an inner join
     * @param exp
     *            an existing expression to AND, or null if none
    ...
    private Expression addJoin(Path path, JPQLNode aliasNode, Expression exp) {
        ...
        if (fmd == null) {
            throw parseException(EX_USER, "path-no-meta", new Object[] { path,
                    null }, null);
        }
        ...
        switch (conf.getCompatibilityInstance().getJPQL()) {
            ...
            case Compatibility.JPQL_STRICT:
                throw new ParseException(
                                         _loc.get("query-extensions-error",
    ...
    /**
     * Assign projections for NEW contructor in selection list. Example: SELECT
     * NEW Person(p.name) FROM Person p WHERE ...
     */
    private Expression assignProjections(JPQLNode parametersNode,
        ...
        for (int i = 0; i < count; i++) {
            ...
            String alias = aliasNode != null ? aliasNode.text
                                            : projectionClause(node.id == JJTSCALAREXPRESSION ? firstChild(node)
                                                                                             : node);
            if (aliasNode != null) {
                proj.setAlias(alias);
            }
    ...
    private Expression assignSubselectProjection(JPQLNode node,
        ...
        exps.projectionClauses[0] = projectionClause(node.id == JJTSCALAREXPRESSION ? firstChild(node)
                                                                                   : node);
    ...
    private JPQLNode child(JPQLNode node, int childNum, int assertCount) {
        if (node.children.length != assertCount) {
            throw parseException(EX_USER, "wrong-child-count",
                                 new Object[] { Integer.valueOf(assertCount),
                                         node, Arrays.asList(node.children) },
                                 null);
        }
    ...
    private Object eval(JPQLNode node) {
        ...
        switch (node.id) {
            ...
            case JJTBOOLEANLITERAL:
                return factory.newLiteral("true".equalsIgnoreCase(node.text) ? Boolean.TRUE
                                                                            : Boolean.FALSE,
            ...
            case JJTINTEGERLITERAL:
                ...
                BigDecimal intlit = new BigDecimal(
                                                   node.text.endsWith("l")
                                                           || node.text.endsWith("L") ? node.text.substring(0,
                                                                                                            node.text.length() - 1)
                                                                                     : node.text).multiply(new BigDecimal(
                                                                                                                          negative(node)));
            ...
            case JJTDECIMALLITERAL:
                BigDecimal declit = new BigDecimal(
                                                   node.text.endsWith("d")
                                                           || node.text.endsWith("D")
                                                           || node.text.endsWith("f")
                                                           || node.text.endsWith("F") ? node.text.substring(0,
                                                                                                            node.text.length() - 1)
                                                                                     : node.text).multiply(new BigDecimal(
                                                                                                                          negative(node)));
            ...
            case JJTBETWEEN: // x.field [NOT] BETWEEN 5 AND 10
                ...
                return evalNot(not,
                               and(factory.greaterThanEqual(val1, val2),
                ...
                while (inIterator.hasNext()) {
                    ...
                    if (isVerticalTypeInExpr(val1, node) && not) {
                        if (inExp == null) {
                            ...
                            inExp = factory.and(inExp,
                                                factory.notEqual(val1, val2));
                        }
                    } else {
                        if (inExp == null) {
            ...
            case JJTISNULL: // x.field IS [NOT] NULL
                ...
                if (not) {
                    return factory.notEqual(val1, factory.getNull());
                } else {
                    return factory.equal(val1, factory.getNull());
                }
            ...
            case JJTLIKE: // field LIKE '%someval%'
                ...
                String escape = null;
                JPQLNode escapeNode = right(node).findChildByID(JJTESCAPECHARACTER,
                                                                true);
                if (escapeNode != null) {
            ...
            case JJTISEMPTY:
                return evalNot(not, factory.isEmpty(getValue(onlyChild(node))));
            ...
            case JJTTRIM: // TRIM([[where] [char] FROM] field)
                ...
                if (node.getChildCount() > 1) {
                    trimWhere = firstTrimChild.id == JJTTRIMLEADING ? Boolean.TRUE
    ...
    private Expression evalFromClause(JPQLNode from, boolean needsAlias) {
        ...
        for (JPQLNode node : from.children) {
    ...
    private void evalGroupingClause(QueryExpressions exps) {
        ...
        for (int i = 0; i < groupByCount; i++) {
            ...
            if (val instanceof Path) {
                ...
                if (fmd != null && fmd.getValue().getTypeMetaData() != null
                    && fmd.getValue().isEmbedded()) {
                    throw parseException(EX_USER,
                                         "cant-groupby-embeddable",
                                         new Object[] { node.getChildCount() > 1 ? assemble(node)
                                                                                : node.text },
                                         null);
                }
            }
        ...
        if (orderby != null) {
            ...
            for (int i = 0; i < ordercount; i++) {
                ...
                exps.ascending[i] = node.getChildCount() <= 1
                                    || lastChild(node).id == JJTASCENDING ? true
                                                                         : false;
            }
            ...
            for (int i = 0; i < ordercount; i++) {
                ...
                for (int j = 0; j < exps.projections.length; j++) {
                    if (exps.projectionAliases[j].equalsIgnoreCase(exps.orderingAliases[i])) {
    ...
    private void evalProjectionsResultShape(JPQLNode selectionsNode,
        ...
        if (count > 1) {
            ...
            resultShape = new ResultShape(
                                          resultClass,
                                          new FillStrategy.Array<Object[]>(
                                                                           Object[].class));
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (node.id == JJTCONSTRUCTOR) {
                ...
                Class<?> constructor = resolver.classForName(resultClassName,
                                                             null);
                if (constructor == null) {
                ...
                for (Value val : terms) {
                    ...
                    cons.nest(new ResultShape(type, new FillStrategy.Assign(),
                                              type.isPrimitive()));
                }
                if (count == 1) {
                ...
                ResultShape<?> projShape = new ResultShape(
                                                           type,
                                                           new FillStrategy.Assign(),
                                                           type.isPrimitive());
    ...
    private Expression evalSelectClause(QueryExpressions exps) {
        ...
        JPQLNode expNode = selectNode.findChildByID(JJTSELECTEXPRESSIONS, true);
        if (expNode == null) {
            return null;
        }
    ...
    private JPQLNode firstChild(JPQLNode node) {
        if (node.children == null || node.children.length == 0) {
            throw parseException(EX_USER, "no-children", new Object[] { node },
                                 null);
        }
    ...
    private ClassMetaData getClassMetaData(String alias, boolean assertValid) {
        ...
        if (cmd == null && assertValid) {
            ...
            if (close != null) {
                throw parseException(EX_USER,
                                     "not-schema-name-hint",
                                     new Object[] { alias, close,
                                             repos.getAliasNames() }, null);
            } else {
                throw parseException(EX_USER, "not-schema-name", new Object[] {
                        alias, repos.getAliasNames() }, null);
            }
        }
    ...
    private Value getGeneralIdentifier(JPQLNode node, boolean verifyEmbeddable) {
        ...
        if (verifyEmbeddable && node.id == JJTKEY && meta != null
            && fld.getKey().isEmbedded() || node.id == JJTVALUE
            && fld.isElementCollection()
            && fld.getElement().getEmbeddedMetaData() != null) {
            // check basic type
            if (node.parent.parent.id == JJTGROUPBY) {
                throw parseException(EX_USER,
                                     "cant-groupby-key-value-embeddable",
                                     new Object[] {
                                             node.id == JJTVALUE ? "VALUE"
                                                                : "KEY",
                                             id.text }, null);
            } else {
     ...
     * determines whether the first argument is used as-is or converted to an
     * Integer as parameter key.
     * 
     * @param the
     *            text as it appears in the parsed node
     * @param positional
     *            if true the first argument is converted to an integer
     * @param isCollectionValued
     *            true for collection-valued parameters
     */
    private Parameter getParameter(String id, boolean positional,
        ...
        Parameter param = isCollectionValued ? factory.newCollectionValuedParameter(paramKey,
                                                                                    TYPE_OBJECT)
                                            : factory.newParameter(paramKey,
                                                                   TYPE_OBJECT);
    ...
    private Path getPath(JPQLNode node, boolean pcOnly, boolean inner) {
        ...
        if (name.equalsIgnoreCase(ctx().schemaAlias)) {
            ...
            throw parseException(EX_USER, "path-invalid", new Object[] {
                    assemble(node), name }, null);
        }
        ...
        for (int i = 1; i < node.children.length; i++) {
            ...
            if (ctx().getParent() != null
                && ctx().getVariable(path.getSchemaAlias()) == null) {
        ...
        if (c != null) {
            ...
            try {
                ...
                if (node.inEnumPath) {
                    throw parseException(EX_USER,
                                         "no-field",
                                         new Object[] { c.getName(), fieldName },
                                         nsfe);
                } else {
                    return getPath(node, false, true);
                }
    ...
    /**
     * Process type_discriminator type_discriminator ::=
     * TYPE(general_identification_variable |
     * single_valued_object_path_expression | input_parameter )
     */
    ...
    /**
     * Returns the left node (the first of the children), and asserts that there
     * are exactly two children.
     */
    ...
    /**
     * An IntegerLiteral and DecimalLiteral node will have a child node of
     * Negative if it is negative: if so, this method returns -1, else it
     * returns 1.
     */
    ...
    private JPQLNode onlyChild(JPQLNode node) throws UserException {
        ...
        if (node.children.length > 1) {
            throw parseException(EX_USER, "multi-children", new Object[] {
                    node, Arrays.asList(node.children) }, null);
        }
    ...
    /**
     * Returns the right node (the second of the children), and asserts that
    ...
    private JPQLNode root() {
        Field field;
        try {
            field = ctx().parsed.getClass().getDeclaredField("root");
        ...
        try {
            return (JPQLNode) field.get(ctx().parsed);
    ...
    /**
     * Trim off leading and trailing single-quotes, and then replace any
     * internal '' instances with ' (since repeating the quote is the JPQL
     * mechanism of escaping a single quote).
     */
    ...
    private boolean useContains(boolean not, Value val1, Value val2,
                                JPQLNode node) {
        if (isVerticalTypeInExpr(val1, node) && not) {
            return false;
        } else {
    ...
    private void validateParameters() {
            ...
            if (key instanceof Number) {
                if (namedParms) {
                    throw new UserException(
                                            _loc.get("mixed-parameter-types",
                                                     resolver.getQueryContext().getQueryString(),
        ...
        if (numericParms) {
            if (!parameterTypes.keySet().contains(1)) {
                throw new UserException(
                                        _loc.get("missing-positional-parameter",
                                                 resolver.getQueryContext().getQueryString(),
                                                 parameterTypes.keySet().toString()));
            }
        }
    ...
    protected Expression evalFetchJoins(QueryExpressions exps) {
        ...
        for (int i = 0; outers != null && i < outers.length; i++) {
            (joins == null ? joins = new TreeSet<String>() : joins).add(getPath(
                                                                                onlyChild(outers[i])).last().getFullName(false));
        }

        JPQLNode[] inners = root().findChildrenByID(JJTINNERFETCHJOIN);
        for (int i = 0; inners != null && i < inners.length; i++) {
            String path = getPath(onlyChild(inners[i])).last().getFullName(false);
            (joins == null ? joins = new TreeSet<String>() : joins).add(path);
            (innerJoins == null ? innerJoins = new TreeSet<String>()
                               : innerJoins).add(path);
        }
    ...
    protected void evalSetClause(QueryExpressions exps) {
        ...
        for (int i = 0; nodes != null && i < nodes.length; i++) {
            ...
            if (path.last().getValue().getEmbeddedMetaData() != null) {
                throw parseException(EX_USER,
                                     "cant-bulk-update-embeddable",
                                     new Object[] { assemble(firstChild(nodes[i])) },
                                     null);
            }

            JPQLNode lastChild = lastChild(nodes[i]);
            Value val = lastChild.children == null ? null
                                                  : getValue(onlyChild(lastChild));
    ...
    protected ClassMetaData getCandidateMetaData(JPQLNode node) {
        ...
        for (JPQLNode element : from.children) {
            JPQLNode n = element;
    ...
    /**
     * Identification variables in JPQL are case insensitive, so lower-case all
     * variables we are going to bind.
     */
    @Override
