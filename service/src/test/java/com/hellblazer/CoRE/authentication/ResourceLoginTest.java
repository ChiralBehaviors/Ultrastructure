 ...
 * 
 */
public class ResourceLoginTest {
    private Model         model;
    ...
    public void testGoodUser() throws Exception {
        ...
        Resource bob = model.getResourceModel().create("Bob", "Test Dummy",
                                                       loginAspect);
        em.persist(bob);
        em.getTransaction().commit();
        em.getTransaction().begin();
        Facet<Resource, ResourceAttribute> loginFacet = model.getResourceModel().getFacet(bob,
                                                                                          loginAspect);

        ResourceAttribute usernameValue = loginFacet.getValue(model.getKernel().getLoginAttribute());
        assertNotNull(usernameValue);
        ResourceAttribute passwordHashValue = loginFacet.getValue(model.getKernel().getPasswordHashAttribute());
        assertNotNull(passwordHashValue);
        usernameValue.setTextValue(username);
        passwordHashValue.setTextValue(Util.md5Hash(password));
        em.getTransaction().commit();

        ResourceAuthenticator authenticator = new ResourceAuthenticator(model);
