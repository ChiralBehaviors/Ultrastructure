...
public class WorkspaceSnapshotTest extends AbstractModelTest {
    ...
    public void testSerializeWorkspaceSnapshot() throws Exception {
        ...
        WorkspaceAuthorization auth = new WorkspaceAuthorization(pseudoScientist,
                                                                 definingProduct,
                                                                 pseudoScientist, em);
        WorkspaceSnapshot snapshot = new WorkspaceSnapshot(definingProduct,
                                                           Arrays.asList(auth), model.getEntityManager());
        ...
        assertEquals(1, deserialized.getRuleforms()
                                    .size());
        assertEquals(auth, deserialized.getFrontier()
                                       .get(0));
    ...
    public void testWorkspaceSnapshot() {
        ...
        WorkspaceAuthorization auth2 = new WorkspaceAuthorization(aLeak,
                                                                  definingProduct,
                                                                  pseudoScientist, em);
        WorkspaceSnapshot snapshot = new WorkspaceSnapshot(definingProduct,
                                                           Arrays.asList(auth,
                                                                                                                         auth2), model.getEntityManager());
        ...
        for (WorkspaceAuthorization a : retrieved) {
            if (a.getRuleform(em) instanceof Agency) {
                assertEquals(pseudoScientist, a.getRuleform(em));
            } else {
                assertEquals(aLeak, a.getRuleform(em));
