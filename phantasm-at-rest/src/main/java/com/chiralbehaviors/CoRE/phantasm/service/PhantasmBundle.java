...
import com.chiralbehaviors.CoRE.phantasm.service.commands.*;
...
public class PhantasmBundle implements ConfiguredBundle<PhantasmConfiguration> {
     ...
     */
    @Override
    public void run(PhantasmConfiguration configuration, Environment environment) throws Exception {
        ...
        environment.jersey()
                   .register(
                   new WorkspaceResource(FacetFields.configureExecutionScope(configuration.getExecutionScope())));

        environment.lifecycle().manage(new AbstractLifeCycle() {
            @Override
            protected void doStart() throws Exception {
                ...
                if (configuration.isClear()) {
                    create.transaction(config -> {
                        ...
                        KernelUtil.initializeInstance(new ModelImpl(txnlCreate), CoreDbConfiguration.CORE,
                                                      "CoRE instance");
                    });
                    log.info("Loading workspace state: {}", configuration.getWorkspaces());
                    LoadWorkspaceCommand.loadWorkspaces(configuration.getWorkspaces(), create);
                    log.info("Loading snapshot state: {}", configuration.getSnapshots());
                    LoadSnapshotCommand.loadSnapshots(configuration.getSnapshots(), create);
                }
    ...
    private void configureAuth(PhantasmConfiguration configuration, Environment environment) {
        ...
        environment.jersey().register(new AuthxResource());
        environment.jersey().register(new AuthValueFactoryProvider.Binder<>(AuthorizedPrincipal.class));
    }

    private void configureCORS(PhantasmConfiguration configuration, Environment environment) {
        if (configuration.isUseCORS()) {
            ...
            filter.setInitParameter(CrossOriginFilter.ALLOWED_ORIGINS_PARAM, Joiner.on(",").join(cors.allowedOrigins));
            filter.setInitParameter(CrossOriginFilter.ALLOWED_METHODS_PARAM, Joiner.on(",").join(cors.allowedMethods));
            filter.setInitParameter(CrossOriginFilter.ALLOWED_HEADERS_PARAM, Joiner.on(",").join(cors.allowedHeaders));
            filter.setInitParameter(CrossOriginFilter.PREFLIGHT_MAX_AGE_PARAM, Integer.toString(cors.preflightMaxAge));
            filter.setInitParameter(CrossOriginFilter.ALLOW_CREDENTIALS_PARAM, Boolean.toString(cors.allowCredentials));
            filter.setInitParameter(CrossOriginFilter.EXPOSED_HEADERS_PARAM, Joiner.on(",").join(cors.exposedHeaders));
            filter.setInitParameter(CrossOriginFilter.CHAIN_PREFLIGHT_PARAM, Boolean.toString(cors.chainPreflight));
