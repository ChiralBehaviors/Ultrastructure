...
public class WorkflowTest extends AbstractModelTest {
	...
	public void testDeploySystem() {
		...
		Product shutdownProcesses = new Product("Shudown Process service",
				"Service for shutting down processes", core);
		em.persist(shutdownProcesses);

		Product deployProcesses = new Product("Deploy Process service",
				"Deploy new processes to containers", core);
		em.persist(deployProcesses);

		Product restartProcesses = new Product("Restart Process service",
				"Service to restart deployed processes", core);
