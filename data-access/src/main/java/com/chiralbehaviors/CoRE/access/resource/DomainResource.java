...
public class DomainResource {
	...
	public byte[] getDomain(@PathParam("version") int version,
		...
		formatter.writeOut(getPersistenceContext().getMetamodel(),
				loc.get("domain-title").toString(), loc.get("domain-desc")
						.toString(), uriInfo.getRequestUri().getPath(), baos);
	...
	public byte[] getModel(@PathParam("version") int version,
			@Context UriInfo uriInfo, @Auth AuthenticatedPrincipal user)
	...
	public byte[] getProperties(@PathParam("version") int version)
			throws IOException {
		Map<String, Object> properties = getPersistenceContext()
				.getProperties();
