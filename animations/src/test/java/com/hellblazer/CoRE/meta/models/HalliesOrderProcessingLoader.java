 ...
 * 
 */
public class HalliesOrderProcessingLoader {
	...
	public Product ship;
	public Product zombie;
	public Product nonExempt;
	public Product shoggoth;
	...
	public HalliesOrderProcessingLoader(EntityManager em) throws Exception {
		this.em = em;
	...
	public void createAgencies() {
		...
		armyOfDarkness = new Agency("Army of Darkness", "Army of Darkness", core);
		em.persist(armyOfDarkness);
		
		admin = new Agency("Administrator", "Estate Administrator", core);
		em.persist(admin);
	}
	...
	public void createLocations() {
		...
		em.persist(euro);
		node21 = new Location("Node 21", "Node 21 in an estate somewhere", core);
		em.persist(node21);
	}

	public void createMetaProtocols() {
		MetaProtocol m1 = new MetaProtocol(deliver, 1,
				notApplicableRelationship, sameRelationship, sameRelationship,
				state, area, core);
		em.persist(m1);
		MetaProtocol m2 = new MetaProtocol(pick, 1, notApplicableRelationship,
				customerType, sameRelationship, area, area, core);
		em.persist(m2);
		MetaProtocol m3 = new MetaProtocol(ship, 1, notApplicableRelationship,
				customerType, anyRelationship, area, area, core);
		em.persist(m3);
		MetaProtocol m5 = new MetaProtocol(fee, 1, notApplicableRelationship,
				salesTaxStatus, sameRelationship, city, anyRelationship, core);
		em.persist(m5);
		MetaProtocol m6 = new MetaProtocol(printPurchaseOrder, 1,
				notApplicableRelationship, anyRelationship, anyRelationship,
				anyRelationship, anyRelationship, core);
		em.persist(m6);
	}
	...
	public void createProducts() {
		...
		em.persist(roomTemp);
		shoggoth = new Product("Shoggoth Process", "Shoggoth Process", core);
		em.persist(shoggoth);
	}
	...
	public void createProductSequencingAuthorizations() {
		...
		ProductParentSequencingAuthorization activateShipFromPrintCustomsDeclaration = new ProductParentSequencingAuthorization(
				core);
		activateShipFromPrintCustomsDeclaration
				.setService(printCustomsDeclaration);
	...
	public void createProtocols() {
		...
		Protocol salesTaxProtocol = new Protocol(fee, nonExemptAgency,
				nonExempt, dc, anyLocation, billingComputer, salesTax,
				sameProduct, core);
		...
		Protocol discountProtocol = new Protocol(fee, externalCust, abc486,
				euro, us, billingComputer, discount, sameProduct, core);
		em.persist(discountProtocol);

		ProtocolAttribute euroDiscount = new ProtocolAttribute(
				discountAttribute, core);
		euroDiscount.setIntegerValue(5);
		euroDiscount.setProtocol(salesTaxProtocol);
		em.persist(euroDiscount);

		Protocol gtuDiscountedPriceProtocol = new Protocol(fee,
				georgeTownUniversity, abc486, dc, us, billingComputer, fee,
				sameProduct, core);
	...
	public void createRelationships() {
		area = new Relationship("Area",
				"A is a member of the economic community B", core, true);
		em.persist(area);
		...
		customerTypeOf = new Relationship("Customer Type Of",
				"A is the customer type of B", core, customerType);
	...
	public void createServices() {
		...
		checkLetterOfCredit = new Product("CheckLetterOfCredit",
				"Check customer letter of credit", core);
		...
		printCustomsDeclaration = new Product("PrintCustomsDeclaration",
				"Print the customs declaration", core);
		...
		deploy = new Product("Deploy", "Deploy the product", core);
		em.persist(deploy);
	}
	...
	public void createStatusCodes() {
		...
		waitingOnCreditCheck = new StatusCode("Waiting on Credit Check",
				"Waiting for credit check to be completed", core);
		...
		waitingOnPurchaseOrder = new StatusCode("Waiting on purchase order",
				"Waiting for purchase order to be completed", core);
		em.persist(waitingOnPurchaseOrder);

		waitingOnPricing = new StatusCode("Waiting on pricing",
				"Waiting for pricing to be completed", core);
		em.persist(waitingOnPricing);

		waitingOnFee = new StatusCode("Waiting on fee calculation",
				"Waiting for fee calculation to be completed", core);
	...
	public void createStatusCodeSequencing() {
		StatusCodeSequencing s = new StatusCodeSequencing(pick,
				waitingOnCreditCheck, available, core);
		em.persist(s);
		...
		s = new StatusCodeSequencing(deploy, available, active, core);
		em.persist(s);

		s = new StatusCodeSequencing(deploy, active, completed, core);
		em.persist(s);

		s = new StatusCodeSequencing(deploy, active, failure, core);
		em.persist(s);
	}
