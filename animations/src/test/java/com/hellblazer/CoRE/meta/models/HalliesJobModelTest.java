 ...
 * 
 */
public class HalliesJobModelTest extends AbstractModelTest {
	...
	public void testNetworkInference() {
		List<LocationNetwork> edges = em
				.createQuery(
						"SELECT edge FROM LocationNetwork edge WHERE edge.inferred = 1",
						LocationNetwork.class).getResultList();
		assertEquals(22, edges.size());

		TypedQuery<LocationNetwork> edgeQuery = em
				.createQuery(
						"select edge FROM LocationNetwork edge WHERE edge.parent = :parent AND edge.relationship = :relationship AND edge.child = :child",
	...
	private List<Job> findAllJobs() {
		TypedQuery<Job> query = model.getEntityManager().createQuery(
