...
import com.chiralbehaviors.CoRE.phantasm.graphql.types.NetworkAttributeAuthorization.NetworkAttributeAuthorizationUpdateState;
...
public interface NetworkAttributeAuthorizationMutations {
    ...
    default NetworkAttributeAuthorization createNetworkAttributeAuthorization(@NotNull @GraphQLName("state") NetworkAttributeAuthorizationState state,
        ...
        state.update(record);
        record.insert();
        return new NetworkAttributeAuthorization(record);
    }
    ...
    @GraphQLField
    default NetworkAttributeAuthorization updateNetworkAttributeAuthorization(@NotNull @GraphQLName("state") NetworkAttributeAuthorizationUpdateState state,
                                                                              DataFetchingEnvironment env) {
        ExistentialNetworkAttributeAuthorizationRecord record = WorkspaceSchema.ctx(env)
                                                                               .create()
                                                                               .selectFrom(Tables.EXISTENTIAL_NETWORK_ATTRIBUTE_AUTHORIZATION)
                                                                               .where(Tables.EXISTENTIAL_NETWORK_ATTRIBUTE_AUTHORIZATION.ID.equal(UUID.fromString(state.id)))
                                                                               .fetchOne();
        state.update(record);
        record.update();
        return new NetworkAttributeAuthorization(record);
