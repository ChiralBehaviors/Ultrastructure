...
@NamedNativeQueries({
		...
		@NamedNativeQuery(name = "relationship" + NAME_SEARCH_SUFFIX, query = "SELECT id, name, description FROM ruleform.existential_name_search('relationship', :queryString, :numberOfMatches)", resultClass = NameSearchResult.class) })
@NamedQueries({
		...
		@NamedQuery(name = FIND_CLASSIFIED_ATTRIBUTE_VALUES, query = "SELECT "
				...
				+ "       RelationshipNetwork network " + "WHERE "
		...
		@NamedQuery(name = GET_CHILD, query = "SELECT n.child "
				+ "FROM RelationshipNetwork n " + "WHERE n.parent = :p "
				+ "AND n.relationship = :r"),
		@NamedQuery(name = GET_ALL_PARENT_RELATIONSHIPS, query = "SELECT n "
				+ "FROM RelationshipNetwork n " + "WHERE n.child = :c"),
...
public class Relationship extends
	...
	public static final String GET_CHILD = "relationship" + GET_CHILDREN_SUFFIX;
	...
	public static final String UNLINKED = "relationship" + UNLINKED_SUFFIX;
	...
	public void link(Relationship r, Relationship child, Agency updatedBy,
		...
		RelationshipNetwork inverse = new RelationshipNetwork(child,
				r.getInverse(), this, inverseSoftware);
		em.persist(inverse);
	}
