...
public class PhantasmCRUD {
	...
	public PhantasmCRUD(Model model) {
		this.model = model;
		create = model.getKernel().getCREATE();
		delete = model.getKernel().getDELETE();
		invoke = model.getKernel().getINVOKE();
		read = model.getKernel().getREAD();
		remove = model.getKernel().getREMOVE();
		update = model.getKernel().getUPDATE();
		apply = model.getKernel().getAPPLY();
	}
	 ...
	 * @param child
	 */
	public ExistentialRuleform addChild(Aspect facet, ExistentialRuleform instance, NetworkAuthorization auth,
		...
		model.getPhantasmModel().link(instance, auth.getRelationship(), child);
		return instance;
	}
	 ...
	 * @param children
	 */
	public ExistentialRuleform addChildren(Aspect facet, ExistentialRuleform instance, NetworkAuthorization auth,
		...
		children.stream().filter(child -> checkREAD(child)).peek(child -> cast(child, auth.getChild()))
				.forEach(child -> model.getPhantasmModel().link(instance, auth.getRelationship(), child));
		return instance;
	}
	...
	public void cast(ExistentialRuleform ruleform, Aspect facet) {
		if (!model.getPhantasmModel().isAccessible(ruleform.getId(), facet.getClassifier().getId(),
				facet.getClassification().getId())) {
			throw new ClassCastException(String.format("%s not of facet type %s", ruleform.getId(), facet));
		}
	}
	...
	public ExistentialRuleform createInstance(Aspect facet, String name, String description,
		...
		instance = model.records().createExistential(facet.getFacet().getClassification(), name, description);
	 ...
	 * @return
	 */
	public Object getAttributeValue(Aspect facet, ExistentialRuleform instance, AttributeAuthorization stateAuth) {
		if (instance == null) {
			return null;
		}
		...
		return model.getPhantasmModel()
				.getValue(model.getPhantasmModel().getAttributeValue(instance, authorizedAttribute));
	}
	 ...
	 * @return
	 */
	public List<ExistentialRuleform> getChildren(Aspect facet, ExistentialRuleform instance,
	 ...
	 * @return
	 */
	public List<ExistentialRuleform> getImmediateChildren(Aspect facet, ExistentialRuleform instance,
			NetworkAuthorization auth) {
		if (instance == null) {
			return Collections.emptyList();
		}
	...
	public List<ExistentialRuleform> getInstances(Aspect facet) {
		...
		return model.getPhantasmModel()
				.getChildrenUuid(facet.getClassification().getId(), facet.getClassifier().getInverse(),
						facet.getDomain())
				.stream().map(e -> e).filter(instance -> checkREAD(instance)).collect(Collectors.toList());
	}
	 ...
	 * @return
	 */
	public ExistentialRuleform getSingularChild(Aspect facet, ExistentialRuleform instance, NetworkAuthorization auth) {
		if (instance == null) {
			return null;
		}
		if (!checkREAD(facet) || !checkREAD(auth)) {
		...
		ExistentialRuleform child = model.getPhantasmModel().getImmediateChild(instance, auth.getRelationship(),
				auth.getDomain());
	 ...
	 * @throws SecurityException
	 */
	public ExistentialRuleform remove(Aspect facet, ExistentialRuleform instance, boolean deleteAttributes)
			throws SecurityException {
		if (instance == null) {
			return null;
		}
	 ...
	 * @param child
	 */
	public ExistentialRuleform removeChild(Aspect facet, ExistentialRuleform instance, NetworkAuthorization auth,
			ExistentialRuleform child) {
		if (instance == null) {
			return null;
		}
		...
		model.getPhantasmModel().unlink(instance, auth.getRelationship(), child);
		return instance;
	}
	 ...
	 * @param children
	 */
	public ExistentialRuleform removeChildren(Aspect facet, ExistentialRuleform instance, NetworkAuthorization auth,
			List<ExistentialRuleform> children) {
		if (instance == null) {
			return null;
		}
	...
	public ExistentialRuleform setAttributeValue(Aspect facet, ExistentialRuleform instance,
			AttributeAuthorization stateAuth, List<Object> value) {
		return setAttributeValue(facet, instance, stateAuth, value.toArray());
	}

	public ExistentialRuleform setAttributeValue(Aspect facet, ExistentialRuleform instance,
			AttributeAuthorization stateAuth, Map<String, Object> value) {
	...
	public ExistentialRuleform setAttributeValue(Aspect facet, ExistentialRuleform instance,
			AttributeAuthorization stateAuth, Object value) {
		if (instance == null) {
			return null;
		}
		if (!checkUPDATE(facet) || !checkUPDATE(stateAuth)) {
		...
		model.getPhantasmModel()
				.setValue(model.getPhantasmModel().getAttributeValue(instance, stateAuth.getAttribute()), value);
		return instance;
	}
	 ...
	 * @param children
	 */
	public ExistentialRuleform setChildren(Aspect facet, ExistentialRuleform instance, NetworkAuthorization auth,
			List<ExistentialRuleform> children) {
		if (instance == null) {
			return null;
		}
		...
		model.getPhantasmModel().unlinkImmediate(instance, auth.getRelationship());
		children.stream().filter(child -> checkREAD(child)).peek(child -> cast(child, auth.getChild()))
				.forEach(child -> model.getPhantasmModel().link(instance, auth.getRelationship(), child));
		return instance;
	}
	 ...
	 * @return
	 */
	public ExistentialRuleform setDescription(ExistentialRuleform instance, String description) {
		if (instance == null) {
			return null;
		}
	...
	public ExistentialRuleform setName(ExistentialRuleform instance, String name) {
		if (instance == null) {
			return null;
		}
		if (!checkUPDATE(instance)) {
	 ...
	 * @param child
	 */
	public ExistentialRuleform setSingularChild(Aspect facet, ExistentialRuleform instance, NetworkAuthorization auth,
			ExistentialRuleform child) {
		if (instance == null) {
			return null;
		}
		...
		if (child == null) {
			...
			cast(child, auth.getChild());
			model.getPhantasmModel().setImmediateChild(instance, auth.getRelationship(), child);
		}
		return instance;
	}
	...
	private ExistentialRecord existential(String id) {
		return model.create().selectFrom(EXISTENTIAL).where(EXISTENTIAL.ID.eq(UUID.fromString(id))).fetchOne();
	}

	private Object[] getIndexedAttributeValue(ExistentialRuleform instance, Attribute authorizedAttribute) {

		ExistentialAttributeRecord[] attributeValues = getValueArray(instance, authorizedAttribute);

		Object[] values = (Object[]) Array.newInstance(model.getPhantasmModel().valueClass(authorizedAttribute),
				attributeValues.length);
		for (ExistentialAttributeRecord value : attributeValues) {
			values[value.getSequenceNumber()] = model.getPhantasmModel().getValue(value);
		}
		return values;
	}

	private Map<String, Object> getMappedAttributeValue(ExistentialRuleform instance, Attribute authorizedAttribute) {
		Map<String, Object> map = new HashMap<>();
		for (Map.Entry<String, ExistentialAttributeRecord> entry : getValueMap(instance, authorizedAttribute)
				.entrySet()) {
			map.put(entry.getKey(), model.getPhantasmModel().getValue(entry.getValue()));
		}
		return map;
	}

	private ExistentialAttributeRecord[] getValueArray(ExistentialRuleform instance, Attribute attribute) {
		List<? extends ExistentialAttributeRecord> values = model.getPhantasmModel().getAttributeValues(instance,
				attribute);
		int max = 0;
	...
	private Map<String, ExistentialAttributeRecord> getValueMap(ExistentialRuleform instance, Attribute attribute) {
		Map<String, ExistentialAttributeRecord> map = new HashMap<>();
		for (ExistentialAttributeRecord value : model.getPhantasmModel().getAttributeValues(instance, attribute)) {
			map.put(value.getKey(), value);
		}
		return map;
	}

	private ExistentialAttributeRecord newAttributeValue(ExistentialRuleform instance, Attribute attribute, int i) {
		ExistentialAttributeRecord value = model.getPhantasmModel().create(instance, attribute);
		value.setSequenceNumber(i);
		return value;
	}

	private void setAttributeArray(ExistentialRuleform instance, Attribute authorizedAttribute, Object[] values) {
		ExistentialAttributeRecord[] old = getValueArray(instance, authorizedAttribute);
		if (values == null) {
			if (old != null) {
	...
	private void setAttributeMap(ExistentialRuleform instance, Attribute authorizedAttribute,
			Map<String, Object> values) {
		Map<String, ExistentialAttributeRecord> valueMap = getValueMap(instance, authorizedAttribute);
		values.keySet().stream().filter(keyName -> !valueMap.containsKey(keyName))
		...
		for (Map.Entry<String, Object> entry : values.entrySet()) {
			...
			if (value == null) {
				value = newAttributeValue(instance, authorizedAttribute, ++maxSeq);
				value.insert();
				value.setKey(entry.getKey());
			}
			model.getPhantasmModel().setValue(value, entry.getValue());
		}
	}

	private void setValue(ExistentialRuleform instance, Attribute attribute, int i, ExistentialAttributeRecord existing,
			Object newValue) {
		if (existing == null) {
			existing = newAttributeValue(instance, attribute, i);
			existing.insert();
		}
		model.getPhantasmModel().setValue(existing, newValue);
	}
}
